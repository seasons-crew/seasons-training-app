...
export function StepBuilderForm({
  ...
  const [advanceMode, setAdvanceMode] = useState<StepAdvanceMode>(step?.advanceMode || "video_end");

  return (
    <form action={action} className="grid gap-3 rounded-md bg-stone-50 p-3">
      ...
      {advanceMode === "timer" ? (
        <label className="grid gap-1 text-xs font-semibold uppercase tracking-[0.12em] text-stone-500 sm:max-w-44">
          Seconds
          <input
      ...
      {advanceMode === "manual" ? (
        <label className="grid gap-1 text-xs font-semibold uppercase tracking-[0.12em] text-stone-500 sm:max-w-sm">
          Button label
          <input
